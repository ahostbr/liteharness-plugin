...
(async () => {
  ...
  const hasReady = await page
    .waitForFunction(() => window.__ready === true, { timeout: READY_TIMEOUT * 1000 })
    .then(() => true)
    .catch(() => false);
  ...
  if (hasReady) {
    ...
    console.log(
      `     Recording will use fallback trim of ${animationStartSec.toFixed(2)}s + 0.5s safety margin.`,
    );
  ...
  console.log(
    `▸ ffmpeg: trim=${resolvedTrim.toFixed(2)}s${TRIM_OVERRIDE !== null ? " (manual)" : " (auto)"}, encode H.264…`,
  );
