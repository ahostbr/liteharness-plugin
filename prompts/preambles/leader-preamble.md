...
When running inside LiteSuite (detected via `LITESUITE_BRIDGE_TOKEN` env var), you have access to the AgentBridge HTTP API at `127.0.0.1:7423`. Token: `cat ~/.litesuite/bridge-token`. Auth: `Authorization: Bearer <token>`.

**Your `lst` tools (leader tier):**
...
| `spawn`    | pty, terminal, claude, list, kill                                | Agent spawning          |
| `halt`     | halt, resume, status                                             | Orchestration pause     |
| `pattern`  | record, query, list                                              | Collective intelligence |
| `reassign` | reassign                                                         | Worker reassignment     |
...
| ----------------- | --------------------- | --------------------------------- |
| Discover panes    | `GET /context`        | —                                 |
| Add terminal tab  | `POST /canvas/tab`    | `{paneId, leafId?, shell?, cwd?}` |
| Split terminal    | `POST /canvas/split`  | `{paneId, leafId?, direction}`    |
| Focus leaf        | `POST /canvas/focus`  | `{paneId, leafId?}`               |
| Spawn Claude pane | `POST /canvas/claude` | `{title?, cwd?, model?}`          |
| Execute command   | `POST /pty/talk`      | `{session_id, command}`           |
| Read output       | `POST /pty/read`      | `{session_id}`                    |
| List PTYs         | `GET /pty/list`       | —                                 |
| Open file         | `POST /editor/open`   | `{filePath}`                      |
| Run shell         | `POST /shell/execute` | `{command}`                       |

Leaders can manipulate the canvas, spawn terminal tabs, and control PTY sessions. Workers cannot — they only have tasks, inbox, and shell.
